...
-- Drop policies for pending_access_requests
...
-- Drop policies for user_permissions
...
-- Drop policies for access_control
...
CREATE POLICY "Service role can manage access control" ON access_control
    FOR ALL TO service_role
    USING (true) WITH CHECK (true);

CREATE POLICY "Users can view access granted to them" ON access_control
    FOR SELECT TO authenticated
    USING (user_id = auth.uid() OR granted_to_user_id = auth.uid());

CREATE POLICY "Authenticated users can manage access control" ON access_control
    FOR ALL TO authenticated
    USING (true) WITH CHECK (true);
...
SELECT 'Database setup completed successfully! All tables, policies, and functions are now ready.' as status;
