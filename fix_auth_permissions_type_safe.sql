-- Fix Auth Permissions - Type Safe Version
-- This script fixes type mismatches between UUID and varchar columns

-- First, let's check and fix column types if needed
-- Note: Run these ALTER statements only if the columns are currently text/varchar

-- Uncomment these lines if your assigned_to and created_by columns are text:
-- ALTER TABLE leads ALTER COLUMN assigned_to TYPE uuid USING assigned_to::uuid;
-- ALTER TABLE leads ALTER COLUMN created_by TYPE uuid USING created_by::uuid;
-- ALTER TABLE opportunities ALTER COLUMN assigned_to TYPE uuid USING assigned_to::uuid;
-- ALTER TABLE opportunities ALTER COLUMN created_by TYPE uuid USING created_by::uuid;

-- Create helper functions that work around auth.users limitations
...
-- Create comprehensive dashboard metrics function with type-safe comparisons
...
BEGIN
  ...
  ELSE
    -- Regular user sees only their data - using type-safe comparisons
    RETURN QUERY
    SELECT 
      (SELECT COUNT(*) FROM leads WHERE 
        (assigned_to IS NOT NULL AND assigned_to::text = current_user_id::text) OR 
        (created_by IS NOT NULL AND created_by::text = current_user_id::text)
      )::bigint as total_leads,
      (SELECT COUNT(*) FROM opportunities WHERE 
        (assigned_to IS NOT NULL AND assigned_to::text = current_user_id::text) OR 
        (created_by IS NOT NULL AND created_by::text = current_user_id::text)
      )::bigint as total_opportunities,
      (SELECT COALESCE(SUM(value), 0) FROM opportunities WHERE 
        ((assigned_to IS NOT NULL AND assigned_to::text = current_user_id::text) OR 
         (created_by IS NOT NULL AND created_by::text = current_user_id::text)) 
        AND stage = 'closed_won'
      )::numeric as total_revenue,
      (CASE 
        WHEN (SELECT COUNT(*) FROM leads WHERE 
          (assigned_to IS NOT NULL AND assigned_to::text = current_user_id::text) OR 
          (created_by IS NOT NULL AND created_by::text = current_user_id::text)
        ) > 0 
        THEN (SELECT COUNT(*) FROM opportunities WHERE 
          ((assigned_to IS NOT NULL AND assigned_to::text = current_user_id::text) OR 
           (created_by IS NOT NULL AND created_by::text = current_user_id::text)) 
          AND stage = 'closed_won'
        )::numeric / (SELECT COUNT(*) FROM leads WHERE 
          (assigned_to IS NOT NULL AND assigned_to::text = current_user_id::text) OR 
          (created_by IS NOT NULL AND created_by::text = current_user_id::text)
        )::numeric * 100 
        ELSE 0 
...
-- Create lead source data function with type-safe comparisons
...
BEGIN
  ...
  ELSE
    ...
    SELECT l.source, COUNT(*)::bigint as count
    FROM leads l
    WHERE (l.assigned_to IS NOT NULL AND l.assigned_to::text = current_user_id::text) OR 
          (l.created_by IS NOT NULL AND l.created_by::text = current_user_id::text)
    GROUP BY l.source
    ORDER BY count DESC;
  END IF;
END;
...
-- Create pipeline data function with type-safe comparisons
...
BEGIN
  ...
  ELSE
    ...
    SELECT o.stage, COUNT(*)::bigint as count, COALESCE(SUM(o.value), 0)::numeric as total_value
    FROM opportunities o
    WHERE (o.assigned_to IS NOT NULL AND o.assigned_to::text = current_user_id::text) OR 
          (o.created_by IS NOT NULL AND o.created_by::text = current_user_id::text)
    GROUP BY o.stage
    ORDER BY total_value DESC;
  END IF;
END;
...
-- Create top opportunities function with type-safe comparisons
...
-- Update RLS policies for leads table with type-safe comparisons
...
-- Create new RLS policies for leads with type-safe comparisons
CREATE POLICY "leads_select_policy" ON leads
  FOR SELECT TO authenticated
  USING (
    is_user_admin() OR 
    (assigned_to IS NOT NULL AND assigned_to::text = auth.uid()::text) OR 
...
-- Update RLS policies for opportunities table with type-safe comparisons
...
-- Create new RLS policies for opportunities with type-safe comparisons
