-- ================================================
-- Simplified Admin Dashboard Fix Using Functions
-- ================================================

-- 1. Drop the problematic view and policies
DROP VIEW IF EXISTS public.dashboard_metrics CASCADE;
DROP POLICY IF EXISTS "Users can view their dashboard metrics" ON public.dashboard_metrics;

-- 2. Update RLS policies to be more permissive for admins
DROP POLICY IF EXISTS "Admin and user lead access" ON public.leads;
DROP POLICY IF EXISTS "Admin and user opportunity access" ON public.opportunities;
...
-- 4. Create a function to get dashboard metrics
...
-- 5. Ensure the lead source and pipeline functions exist
...
SELECT 
    u.email,
    ur.role,
    'This should show your role' as note
FROM auth.users u
LEFT JOIN public.user_roles ur ON u.id = ur.user_id
WHERE u.id = auth.uid();

-- Success message
SELECT 'Simplified admin dashboard access has been configured!' AS status;
